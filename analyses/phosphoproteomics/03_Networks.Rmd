...
### Union networks ----
Combine perturbation networks for each organoid.
...
targets <- read.table("../../tables/targets.tsv", header = T)
SGL$crc.graphs <- SGL$crc.graphs %L>% mutate(target = ifelse(name %in% targets$target, "direct", "not direct"))

gene2inh <- unique(targets$target) %S>% function(tar){
  paste0(subset(targets, target == tar)$inhibitor, collapse = "|")
}
SGL$crc.graphs <- SGL$crc.graphs %L>% mutate(inhibitor = gene2inh[name])

inh_colors <- setNames(colors$treatment[unique(gene2inh)], unique(gene2inh))
inh_colors["BRAFi|TAKi"] <- "#ebc05e"
inh_colors["mTORi|PI3Ki"] <- "#fc1e4b"

target_color <- "#f2a807"
base_color <- rgb(0.4, 0.4, 0.4)
title_size <- 19
axis_textsize <- 14
axis_labsize <- 19
point_label_size <- 12
...
SGL$crc.graphs <- SGL$crc.graphs %L>% function(x){ V(x)$label[is.na(V(x)$inhibitor) & V(x)$mutation == "not mutated"] <- ""; x }

ggg <- SGL$crc.graphs %L>% ggnet(node_mapping = aes(color = inhibitor, fill = inhibitor, shape = mutation),
...
ggg %<L>% function(g){ g + scale_color_manual("node type", values = inh_colors, na.value = rgb(0.45, 0.45, 0.45)) +
                           scale_fill_manual("node type", values = inh_colors, na.value = "white") +
...
ggsave(gg_net, filename = file.path(params$figures, "Fig4a.pdf"), bg = "white", device = "pdf", width = 12, height = 8)

get_legend(ggg$CRC04) |>
  ggsave(filename = file.path(params$figures, "Fig4a_legend.pdf"), bg = "white", device = "pdf", width = 4, height = 3)
...
ggsave(gg, filename = file.path(params$figures, "SuppFig4.jpeg"), bg = "white", device = "jpeg", width = 9, height = 11)
ggsave(ggleg, filename = file.path(params$figures, "SuppFig4_legend.jpeg"), bg = "white", device = "jpeg", width = 2, height = 6)
...
target_res <- names(SGL$crc.graphs) %S>% function(crc){
  
  x <- setNames(V(SGL$IG)$name %in% targets$target, V(SGL$IG)$name)
  y <- setNames(V(SGL$crc.graphs[[crc]])$is_target, V(SGL$crc.graphs[[crc]])$name) 
  
  contable <- matrix(nrow=2, ncol=2)
  rownames(contable) <- c("subnet", "global")
  colnames(contable) <- c("target", "notarget")
  contable["subnet", "target"] <- sum(y)
  contable["subnet", "notarget"] <- sum(!y)
  contable["global", "target"] <- sum(x[!names(x) %in% names(y)], na.rm = T)
  contable["global", "notarget"] <- sum(!x[!names(x) %in% names(y)], na.rm = T)
  
  fisher.test(contable, alternative="two.sided")$p.value
}

write.table(data.frame(organoid = names(target_res), pval = target_res),
            file = file.path(params$results, "Subgraph_targets_fisher.txt"), quote = F, row.names = F)
