...
site_plots <- names(measured_sites) %L>% function(tmp){
  ...
  plotdata$phosphosite <- sub("_", "-", plotdata$phosphosite)
  
  desites <- unique(plotdata$phosphosite[plotdata$DE == "sig"])
  plotdata <- subset(plotdata, phosphosite %in% desites)

  if (tmp == "TNFalpha") tmp <- "TNFa"
  
  gg <- ggplot(data = plotdata, aes(x = phosphosite, y = log2FC, color = log2FC, fill = log2FC, shape = DE)) +
    theme_bw(base_size = 22) + xlab(NULL) +
    theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
          legend.title = element_blank(),
          legend.margin = margin(0.5, 1, 1, 1),
          strip.background =  element_rect(colour = NA, fill = NA, size=1),
          axis.title = element_text(size = 22),
          strip.text = element_text(size = 22),
          axis.text.x = element_text(color = rgb(0,0,0), 
                                     angle = 90, hjust = 1, vjust = 0.5, size = 12,
                                     margin = margin(t = 0, r = 0, b = 0, l = 0, unit = "npc")),
          plot.title = element_text(hjust = 0.5),
          plot.margin = margin(2, 2, 8, 0, unit = "pt"),
          legend.position = "right") +
    geom_segment(aes(xend=phosphosite, y=0, yend=log2FC), size = 1.5) +
    ...
    scale_color_gradientn(colors = c("#5106b2", rgb(0.94,0.94,0.94), "#f2c80e"), values = scales::rescale(c(-9,0,3)), limits = c(-9, 3), breaks = c(-8, -6, -4, -2, 0, 2)) +
    scale_fill_gradientn(colors = c("#5106b2", rgb(0.94,0.94,0.94), "#f2c80e"), values = scales::rescale(c(-9,0,3)), limits = c(-9, 3), breaks = c(-8, -6, -4, -2, 0, 2)) +

    ylim(1.2*c(floor(min(plotdata$log2FC)), max(ceiling(plotdata$log2FC), 0.2))) # + guides(color = "none", fill = "none")
...
site_plot <- patchwork::wrap_plots(site_plots, ncol = 1, guides = "collect")
cowplot::get_legend(site_plot & theme(legend.direction = "horizontal", legend.key.width = unit(1, "in"))) |>
  ggsave(path = params$figures, filename = "SuppFig3b_legend.pdf", device = "pdf", units = "in", width = 10, height = 5, dpi = 300)

ggsave(path = params$figures, filename = "SuppFig3b.pdf",
       plot = site_plot & theme(legend.position = "none"), device = "pdf", units = "in", width = 16, height = 18, dpi = 300)
