...
if (!dir.exists(params$resultsdir_rep)){ dir.create(params$resultsdir_rep) }
...
fontsize <- 11
topann <- HeatmapAnnotation(df = data.frame(organoid = gsub("_.*", "", colnames(NES)), treatment = gsub(".*_", "", colnames(NES)))[,-2,drop=F],
                           annotation_height = unit(20/2, "points"),
                           gap = unit(0, "cm"),
                           border = FALSE,
...
hm <- ComplexHeatmap::Heatmap(matrix = NES,
                              ...
                              column_labels = sub("_.*", "", colnames(NES)),
...
pdf(file = file.path(params$figures, "Fig3c.pdf"), onefile = TRUE, width = 15, height = 7)
...
cldata[is.na(cldata)] <- 0
rowcl <- hclust(dist(cldata))
rownames(tmp) <- cutNames(rownames(tmp), maxchar = 87)
hm <- ComplexHeatmap::Heatmap(matrix = tmp,
                              column_labels = sub("_.*", "", colnames(tmp)),
                              top_annotation = topann,
                              row_names_gp = gpar(fontsize = 7), row_names_max_width = unit(0.6,"npc"),
                              column_names_gp = gpar(fontsize = 9),
                              cell_fun = signiFUN,
                              name = "NES",
                              column_split = sub(".*_", "", colnames(NES)),
...
pdf(file = file.path(params$figures, "Fig3d.pdf"), width = 15, height = 7, bg = "white")
  ComplexHeatmap::draw(hm, annotation_legend_list  = list(lgd))
dev.off()
...
ggsave(gg, filename = file.path(params$figures, "SuppFig3d.pdf"), device = "pdf", width = 9, height = 10)
...
## Alternative analysis using organoids as replicates

```{r}

mm <- model.matrix(~ 0 + treatment * organoid, phosphodata$design)
colnames(mm) <- make.names(colnames(mm))
fit <- limma::lmFit(log2(phosphodata$data + 1)[,rownames(mm)], mm)
trt <- grep("treatment", colnames(fit$coefficients), value = TRUE) |> grep(pattern = "organoid", invert = TRUE, value = TRUE)

contrasts <- lapply(trt[-1], function(trtx){
  paste0(trtx ," - ", trt[1]) |> setNames(trtx) |> as.list()
}) |> unlist(recursive = FALSE)
contrasts_limma <- do.call(what = limma::makeContrasts, args = c(contrasts, list(levels = mm)))
cfit <- limma::contrasts.fit(fit = fit, contrasts = contrasts_limma)
efit <- limma::eBayes(fit = cfit, trend = TRUE)
trt <- grep("treatment", colnames(efit$coefficients), value = TRUE)
res_paired <- lapply(setNames(trt, trt), function(trtx){
  topTable(efit, coef = trtx, number = Inf)
})

saveRDS(res_paired, file.path(params$resultsdir_rep, "res_paired.rds"))
...
scores <- sapply(res_paired, function(x){
  score <- -log10(x$P.Value) * sign(x$logFC) |> setNames(rownames(x))
  score[phosphodata$annotation$psite_name] |> setNames(phosphodata$annotation$psite_name_protein)
})
...
ix <- order(rowMeans(abs(scores)), decreasing = TRUE)
scores <- scores[ix,]
scores <- scores[!duplicated(rownames(scores)),]

sort(table(rownames(scores)), decreasing = TRUE)[1:5]

inputGCT <- new("GCT", mat = data.matrix(scores))
cmapR::write_gct(ds = inputGCT, ofile = file.path(params$resultsdir_rep, "PTMSEA_trt", "input_uniprot.gct"), appenddim = FALSE, precision = 8)
...
cd $resultsdir_rep/PTMSEA_trt

qsub -pe smp $sgecores $ptmseaR \
  --script $ssgsea \
  --input input_uniprot.gct \
  --db ../PTMSEA/ptmsets_combined.gmt \
  --ouptut ptmsea \
  --test area.under.RES \
  --weight 1 \
  --norm none \
...
PTMSEAresults <- c("NES" = "ptmsea-scores.gct", "pval" = "ptmsea-pvalues.gct", "FDR" = "ptmsea-fdr-pvalues.gct") %L>%
  function(file){ mat(parse_gctx(fname = file.path(params$resultsdir_rep, "PTMSEA_trt", file))) }

metares <- meta(parse_gctx(fname = file.path(params$resultsdir_rep, "PTMSEA_trt", "ptmsea-scores.gct")))
...
writeTables(PTMSEAresults, file.path(params$resultsdir_rep, "PTMSEA_trt", "PTMSEA_results.xlsx"), rownames = T)
saveRDS(object = PTMSEAresults, file = file.path(params$resultsdir_rep, "PTMSEA_trt", "PTMSEAresults.rds"), compress = "gzip")

```


```{r}
PTMSEAresults <- readRDS(file.path(params$resultsdir_rep, "PTMSEA_trt", "PTMSEAresults.rds"))

ix <- rowSums(PTMSEAresults$FDR <= 0.05)
filter <- !grepl("P100", rownames(PTMSEAresults$FDR))
tmp <- PTMSEAresults$NES[ix > 0 & filter,]
colnames(tmp) <- sub("treatment", "", colnames(tmp))
mat <- (PTMSEAresults$FDR <= 0.05)[ix > 0 & filter,]
colnames(mat) <- sub("treatment", "", colnames(mat))

df <- data.frame(row.names = rownames(mat), source = ifelse(grepl("PTMSigDB", rownames(mat)), "PTMSigDB", "SIGNOR"))
df$class <- sub("_.*", "", rownames(mat))
df$class[grep("SIGNOR", df$class)] <- "kinases (SIGNOR)"
df$class[df$class == "KINASE-PSP"] <- "kinases (PTMSigDB)"
df$class[df$class == "PERT-PSP"] <- "perturbations (PTMSigDB)"
df$class[df$class %in% c("PATH-NP", "PATH-WP", "")] <- "pathways (PTMSigDB)"
df$label <- sub(".*P_", "", rownames(df))
df$label <- gsub("\\(.*", "", df$label) |> trimws()
df <- arrange(df, label)
...
heatdata <- t(tmp)
cellmat <- t(mat)[rownames(heatdata), colnames(heatdata)]
cellFUN <- function(j, i, x, y, width, height, fill) {
            if (naf(cellmat[i, j] == TRUE)) {
                grid::grid.points(x, y, pch = 16, size = unit(3, "pt"))
            }
}

coldf = df[,2,drop=FALSE]
legend_params <- list(title_gp = grid::gpar(fontsize = 8, 
        fontface = "bold"), legend_height = grid::unit(0.2, "npc"), 
        labels_gp = grid::gpar(fontsize = 8))
colAnn <- datamisc:::getCXanno(coldf[colnames(heatdata), , drop = FALSE], 
            colors =  datamisc::getColors(coldf), legend_params = legend_params)
colAnn@height <- grid::unit(3, "mm")

ComplexHeatmap::Heatmap(heatdata, name = "NES", cell_fun = cellFUN, column_labels = df$label, column_split = df$class, cluster_columns = TRUE,
                        top_annotation = colAnn, 
        column_names_max_height = grid::unit(10, "npc"),
        rect_gp = grid::gpar(col = rgb(1, 1, 1), lwd = grid::unit(1, "pt")),
        column_title_gp = grid::gpar(fontsize = 8, fontface = "bold"),
        row_names_gp = grid::gpar(fontsize = 8),
        column_names_gp = grid::gpar(fontsize = 6)) |>
  datamisc::saveplot(file.path(params$resultsdir_rep, "ptmsea_paired_treatments_split"), dev = "pdf", width = 3500, height = 1000)
